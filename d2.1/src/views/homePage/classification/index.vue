<template>
  <div class="container">
          ...
          >
            <template slot-scope="scope">
              ...
              <el-button
                ...
                size="mini"
                type="primary"
                @click="handleRelated(scope.row, 'related')"
                >影片关联</el-button
              >
              <el-button
                class="btn_change_handle"
                size="mini"
                type="info"
                @click="handleRelated(scope.row, 'cancle')"
                >取消关联</el-button
              >
              <el-button
                class="btn_change_handle"
    ...
    <!-- 影片关联 -->
    <related :show="isAss" :type="filmRelatedType" ref="related" />
    <!-- 日志关联 -->
    <console :show="isConsole" ref="console" />
  </div>
</template>
<script>
...
import console from "./modal/console.vue";
...
export default {
  name: "Classification",
  components: { pagination, newAddClass, related, console },
  data() {
    return {
      loading: false,
      list: [],
  ...
  methods: {
    ...
    handleConsole() {
      this.isConsole = true;
      this.$refs["console"].getRelatedConsole();
    },
    // 新增分类
    handleNewAdd() {
      this.isAdd = true;
    ...
    handleShelf(row) {
      ...
      if (!row.status) {
        this.$confirm(
          `该分类下有 <span style="color: #2EB5F0">${row.moveNum}</span> 个影片，确定上架该影片类型吗？`,
          "提示",
          {
        ...
        this.$confirm(
          `该分类下有 <span style="color: #2EB5F0">${row.moveNum}</span> 个影片，确定下架该影片类型吗？下架后前端用户不可见`,
          "提示",
          {
            confirmButtonText: "确定",
            cancelButtonText: "取消",
    ...
    // 影片关联
    handleRelated(row, type) {
      this.filmRelatedType = type;
      this.isAss = true;
      ...
      RelatedList({
        ...
        .then((res) => {
          if (res.code * 1 === 200) {
            ...
            if (type === "related") {
              this.array = no;
              this.handleSlicePage(pageNum, pageSize, this.array);
            } else {
              this.array = yes;
              this.handleSlicePage(pageNum, pageSize, this.array);
            }
    ...
    // 分页
    handleSlicePage(num, size, item) {
      let { page, pageSize, data, length } = getTableData(num, size, item);
      ...
      this.$nextTick(() => {
        item.forEach((row) => {
          this.$refs["related"].allSelect = row.isRelated;
          if (row.isRelated) {
            this.$refs["related"].$refs.multipleTable.toggleRowSelection(
              row,
              true
            );
          }
