      ...
      userPayCount()
        .then((res) => {
          if (res.code * 1 === 200) {
            ...
            // 今日付费用户
            this.baseList[2].userNums = todayPayCount;
            this.baseList[2].iosNums = iosTodayPayCount;
            this.baseList[2].androidNums = azTodayPayCount;

            // 总开通会员用户
            this.baseList[3].userNums = vipUser;
            this.baseList[3].iosNums = iosVipUser;
            this.baseList[3].androidNums = azVipUser;

            // 总购买金币用户
            this.baseList[4].userNums = sumCoinUser;
            this.baseList[4].iosNums = iosSumCoinUser;
            this.baseList[4].androidNums = azSumCoinUser;
          }
        })
        .catch(() => {});
    },
          ...
          if (res.code * 1 === 200) {
            switch (this.idx) {
              case "0":
                ...
                this.payChartData.expectedData = userPayTimeVoList.map(
                  (item) => {
                    return item.count;
                  }
                );
