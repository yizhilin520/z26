  ...
  <div class="container">
    <main class="main">
      ...
      <div class="film_content">
        <div class="film_top">
          <h3>帖子列表</h3>
          <el-button type="primary" size="small" plain @click="handleView(false)"
            >发布帖子</el-button
          >
        </div>
        <!-- 列表 -->
    ...
    <div />
    <!-- 发布 -->
    <post-modal :show="isPost" ref="publish" />
    <!-- 详情 -->
...
<script>
import { postList, optionList, remove, postInfo } from "@/api/post_api";
...
export default {
  name: "PostList",
  components: { pagination, postModal, postView, playFilm },
  data() {
    return {
      loading: false,
      userForm: {
  ...
  methods: {
    ...
    getOptionList() {
      optionList()
        .then(res => {
          if (res.code * 1 === 200) {
            this.$refs["publish"].nickOptions = res.data;
            this.isPost = true;
          }
        })
        .catch(e => console.error(e));
    ...
    delete(item) {
      remove({ pid: item.pid })
        .then(res => {
          if (res.code * 1 === 200) {
            if (this.dataList.length === 1 && this.pages.pageNum > 1)
