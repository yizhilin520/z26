...
{
  /* <Selectes  listClickIndex={listClick1}/> */
}
interface Props {
  listClickIndex:any
}
...
export default function CodeSelector(props: Props) {
  let CodeDate =  [
    {
      "english_name": "China",
      "chinese_name": "中国",
  ...
  // console.log('数据', CodeDate)
  
  const {listClickIndex} = props;
 

  const [dropDown, setDropDown] = useState(false);
  const [pList, setpList] = useState(CodeDate[0].phone_code);
  ...
  const listClick = (item: any) => {
    ...
    const phoneCode = item.phone_code
    setpList(phoneCode);
    setDropDown(!dropDown);
    listClickIndex(phoneCode) // 向父组件传值
  };
  ...
  return (
     <div className={`dropdownlist ${dropDown ? 'show1' : ''}`}>
      <div className='drop-text' onClick={handleClick}>
        <p className='drop-value2'>+{pList? pList :''}</p>
        <span className='drop-icon2'></span>
      </div>
      <ul className={`dropUl ${dropDown ? 'in' : ''}`} style={{width: '310px'}}>
        {CodeDate && CodeDate.length > 0 &&
          CodeDate.map((item, index) => {
            return (
              <li key={index} onClick={e => listClick(item)}>
                <span className='textleft'>{item.chinese_name}</span> <span>{item.phone_code}</span>
              </li>
            );
          })}
      </ul>
