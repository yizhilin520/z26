...
import { getkickOutUser } from '@/servers/userServer';
import Select from '@/components/Select';
import { HttpCode } from '@/enums';
import Dialog from './Dialog';
import Form from './Form';

const AddKick = forwardRef((({ onSubmit }, ref) => {
  ...
  const onSubmitHandle = async () => {
    ...
    if (typeof forbidTimeType === 'undefined') return setMessage({ forbidTimeType: '踢人时长没有选择' });
    if (typeof violateType === 'undefined') return setMessage({ violateType: '违规类型没有选择' });

    const params = { account, forbidTimeType, violateType, roomId, optType: 2 };
  ...
  return (
    <Dialog title="新增踢人" visible={visible} onClose={onCloseHandle}>
      <Form>
...
AddKick.defaultProps = {
  onSubmit: empty
};

export default AddKick;
