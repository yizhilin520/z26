 ...
 * @returns {string}
 */
export function formatSecond(second, options) {
  var hourTime = 0;
  var minuteTime = 0;
  var secondTime = 0;
  ...
  if (options && options == "1") {
    result = "" + Math.round(secondTime) + "秒";
    // if (minuteTime > 0) {
    result = "" + Math.round(minuteTime) + "分" + result;
    // }
    if (hourTime > 0) {
      result = "" + Math.round(hourTime) + "时" + result;
    }
  } else {
    result =
      _addZero(hourTime) +
...
export function getTableData(page = 1, pageSize = 10, totalData = []) {
  ...
  if (pageSize >= len) {
    ...
    if (num < len) {
      ...
      const size = parseInt(len / pageSize); //取商
      const rest = len % pageSize; //取余数
      if (rest > 0) {
        //余数大于0，说明实际最后一页数据不足pageSize，应该取size+1为最后一条的页码
        tableData.page = size + 1; //当前页码重置，取size+1
        tableData.data = totalData.filter(
          (_, index) => index >= pageSize * size && index <= len
        );
      } else if (rest === 0) {
        //余数等于0，最后一页数据条数正好是pageSize
        tableData.page = size; //当前页码重置，取size
        tableData.data = totalData.filter(
          (_, index) => index >= pageSize * (size - 1) && index <= len
        );
      } //注：余数不可能小于0
    }
  }
