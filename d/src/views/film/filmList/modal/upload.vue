    ...
    >
      ...
      <el-form-item label="">
        ...
        >
          ...
          <br />
          <div class="cover_img" >
            <img v-if="uploadForm.coverPath" :src="uploadForm.coverPath" class="cover_avatar" />
          </div>
        </el-upload>
      </el-form-item>
      ...
      <el-form-item class="label">
        ...
        <p class="label_text tip">最少选1个，最多可同时选择8个</p>
        <el-checkbox-group :max="8" @change="handleCheckbox" v-model="tagList">
          <el-checkbox
...
export default {
  ...
  methods: {
    ...
    handleUploadFilm() {
      if (this.isEdit) {
        ...
        if (this.fileList.length <= 0) {
          ...
          //  else if (!bolList.includes(true)) {
          //   this.$message.error("上传文件不全，请检查");
          // }
          else {
            this.uploadFile();
          }
    ...
    // 长影片编辑
    hanleFilmEdit() {
      let { title, labelTag, id, coverPath } = this.uploadForm;
      let params = {
        labelIds: labelTag,
        title,
        id,
