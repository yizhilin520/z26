...
<script>
import { moveList, postInfo, updateStatus, deleteMove } from "@/api/film_api";
...
export default {
  ...
  methods: {
    ...
    handleIsUp(row) {
      updateStatus({ mid: row.mid, valid: !row.valid })
        .then((res) => {
          if (res.code * 1 === 200) {
            this.fetchFilmData();
          }
    ...
    handleUpload(bool, row) {
      ...
      if (bool) {
        let { title, coverPath, labelIds, mid } = row;
