<template>
  ...
  >
    ...
    >
      ...
      >
        ...
        <div class="e_text">
          <span class="font-warn"
            >* 只能上传大于10秒且小于300M视频，最多10个视频</span
          >
        </div>
      </el-upload>
      ...
      <el-form-item>
        <p class="label_text">选择勾选标签</p>
        <p class="label_text tip">最少选1个，最多可同时选择3个</p>
        <el-checkbox-group
          :max="3"
          @change="handleCheckbox"
          v-model="labelTagList"
        >
          <el-checkbox
...
export default {
  ...
  methods: {
    ...
    beforeUploadVideo(file) {
      const self = this;
      const isLt30MB = file.size / 1024 / 1024 < 300;
      ...
      if (this.fileList.length == 1) {
        videoElement.addEventListener("loadedmetadata", function (_event) {
          ...
          self.upruleForm.duration = duration;

          if (!isLt30MB || Math.floor(duration) < 10) {
            self.$message.error(
              "上传视频时长不能小于 10S, 大小不能超过300MB！" + file.name
            );
            self.isUpload = true;
          }
          return false;
    ...
    handleUploadFilm() {
      ...
      if (this.isShowUpload) {
        ...
        if (this.fileList.length == 1) {
          if (this.isUpload || this.upruleForm.duration == 0) return false;
        } else {
