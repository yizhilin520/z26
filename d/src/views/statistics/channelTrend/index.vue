...
<script>
import { channelRetentionPolyline, channelRemainList} from "@/api/statistics_api";
...
export default {
  ...
  methods: {
    ...
    fetchPolyline() {
      ...
      channelRetentionPolyline({ ...params })
        .then(res => {
          if (res.code * 1 === 200) {
            switch (this.idx) {
              case "0":
                let { nextDayAzUser, nextDayIosUser } = res.data;
                this.channelChartData.expectedData = nextDayIosUser.map(
                  item => {
                    return item.count;
                  }
                );
                this.channelChartData.dateData = nextDayAzUser.map(item => {
                  return `${item.hour >= 10 ? item.hour : "0" + item.hour}点`;
                });
                this.channelChartData.actualData = nextDayAzUser.map(item => {
                  return item.count;
                });
                break;
              case "1":
                let { sevenDayIosUser, sevenDayAzUser } = res.data;
                this.channelChartData.expectedData = sevenDayIosUser.map(
                  item => {
                    return item.count;
                  }
                );
                this.channelChartData.dateData = sevenDayAzUser.map(item => {
                  return item.createTime;
                });
                this.channelChartData.actualData = sevenDayAzUser.map(item => {
                  return item.count;
                });
                break;
              case "2":
                let { thirtyDayAzUser, thirtyDayIosUser } = res.data;
                this.channelChartData.expectedData = thirtyDayIosUser.map(
                  item => {
                    return item.count;
                  }
                );
                this.channelChartData.dateData = thirtyDayAzUser.map(item => {
                  return item.createTime;
                });
                this.channelChartData.actualData = thirtyDayAzUser.map(item => {
                  return item.count;
                });
                break;
